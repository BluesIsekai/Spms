...
import {
    deleteTrendWatchlistSymbol,
    fetchTrendWatchlist,
    upsertTrendWatchlistSymbol,
...
function rsiLabel(rsi) {
    ...
    if (rsi >= 70)
        return { label: `${rsi.toFixed(1)} — Overbought`, color: "#FF5252", note: "Stock may be due for a pullback" };
...
function MiniSparkline({ prices, color = "#00D09C" }) {
    ...
    return (
        <svg viewBox={`0 0 ${W} ${H}`} width="100%" height={H} preserveAspectRatio="none" className="ta-spark">
            ...
            <polygon fill={`url(#tagrad-${color.replace("#", "")})`} points={`0,${H} ${pts} ${W},${H}`} />
        ...
        <div className="ta-card" style={{ borderLeftColor: color }}>
            ...
            <div className="ta-card-header">
                ...
                <div className="ta-card-header-right">
                    ...
                    {onRemove && (
                        <button className="ta-remove" onClick={onRemove}>
                            ✕
                        </button>
                    )}
                </div>
                ...
                <>
                    ...
                    <div className="ta-price-row">
                        <div>
                            ...
                            <div className={`ta-range-ret ${data.rangeReturn >= 0 ? "up" : "down"}`}>
                                {data.rangeReturn >= 0 ? "+" : ""}
                                {data.rangeReturn.toFixed(2)}% ({range})
                            </div>
                        </div>
                        <div
                            className="ta-trend-badge"
                    ...
                    <div className="ta-metrics">
                        ...
                        <div className="ta-metric-block">
                            ...
                            <div className="ta-metric-val" style={{ color: rsiInfo.color }}>
                                {rsiInfo.label}
                            </div>
                        ...
                        <div className="ta-metric-block">
                            ...
                            <div
                                className="ta-metric-val"
                                style={{ color: data.vol > 3 ? "#FF5252" : data.vol > 1.5 ? "#FFD740" : "#00C853" }}
                            >
                                {data.vol.toFixed(2)}%
                            </div>
                        ...
                        {data.macd && (
                            <div className="ta-metric-block">
                                ...
                                <div
                                    className="ta-metric-val"
                                    style={{ color: data.macd.macd >= 0 ? "#00C853" : "#FF5252" }}
                                >
                                    {data.macd.macd >= 0 ? "📈 Bullish" : "📉 Bearish"}
                                </div>
                                <div className="ta-metric-note">
                                    MACD: {data.macd.macd.toFixed(2)} (EMA12 {data.macd.ema12.toFixed(2)} vs EMA26{" "}
                                    {data.macd.ema26.toFixed(2)})
                                </div>
                            </div>
                        )}
                        ...
                        <div className="ta-metric-block ta-signal-block">
                            ...
                            {(() => {
                                ...
                                if (data.sma20 !== null) {
                                    if (data.price > data.sma20) bull++;
                                    else bear++;
                                }
                                if (data.sma50 !== null) {
                                    if (data.price > data.sma50) bull++;
                                    else bear++;
                                }
                                if (data.macd) {
                                    if (data.macd.macd > 0) bull++;
                                    else bear++;
                                }
                                ...
                                const signal =
                                    pctBull >= 70
                                        ? { label: "🚀 Strong Bullish", color: "#00C853" }
                                        : pctBull >= 55
                                          ? { label: "📈 Mildly Bullish", color: "#69F0AE" }
                                          : pctBull <= 30
                                            ? { label: "🔴 Strong Bearish", color: "#FF1744" }
                                            : pctBull <= 45
                                              ? { label: "📉 Mildly Bearish", color: "#FF8A80" }
                                              : { label: "➡️ Neutral", color: "#FFD740" };
                                return (
                                    <>
                                        <div className="ta-metric-val" style={{ color: signal.color }}>
                                            {signal.label}
                                        </div>
...
export default function TrendAnalysis() {
    ...
    useEffect(() => {
        ...
        async function loadTrendPreferences() {
            if (!user?.id) {
                if (!cancelled) {
                    setWatchList([]);
                }
            ...
            try {
                const watchRows = await fetchTrendWatchlist(user.id);
    ...
    const addToWatch = async (symbol, name) => {
        ...
        if (!watchList.find(w => w.symbol === symbol)) {
            ...
            if (user?.id) {
                try {
                    await upsertTrendWatchlistSymbol({ userId: user.id, symbol, displayName: disp });
                } catch (err) {
                    console.error("Failed to save trend watchlist symbol:", err);
                }
            }
        }
        setSearchQ("");
    ...
    const removeFromWatch = async symbol => {
        setWatchList(prev => prev.filter(w => w.symbol !== symbol));
        if (user?.id) {
            try {
                await deleteTrendWatchlistSymbol(user.id, symbol);
            } catch (err) {
                console.error("Failed to delete trend watchlist symbol:", err);
            }
        }
    };

    const handleRemove = async symbol => {
        await removeFromWatch(symbol);
    };
    ...
    return (
        <div className="ta-page">
            <div className="ta-hero">
                ...
                <p className="ta-sub">
                    Technical indicators for your portfolio stocks — RSI, SMA, MACD, Support & Resistance
                </p>
            </div>
            ...
            {allSymbols.length === 0 ? (
                <div className="ta-empty">
                    ...
                    <div>Search for a stock above and add it to see trend analysis.</div>
                </div>
            ) : (
                <div className="ta-grid">
                    {allSymbols.map(({ symbol, displayName }) => (
                        <TrendCard
                            key={symbol}
                            symbol={symbol}
