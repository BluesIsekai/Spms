...
export default function Watchlist({ onOpenChart }) {
  ...
  const handleQuickTrade = async (symbol, action, livePrice) => {
    if (!livePrice) return alert("Market price unavailable.");
    const qty = parseInt(prompt(`Enter quantity to ${action} ${symbol} at ₹${livePrice.toFixed(2)}:`), 10);
    if (!qty || qty <= 0) return;
      ...
      render: (_, row) => (
        <div className="actions-row">
          <Button
            variant="success"
            size="sm"
            onClick={() => handleQuickTrade(row.rawSymbol, 'BUY', row.rawPrice)}
          >
            Buy
          </Button>
          <Button
            variant="danger"
            size="sm"
            onClick={() => handleQuickTrade(row.rawSymbol, 'SELL', row.rawPrice)}
          >
            Sell
          </Button>
          <Button
