...
export default function Watchlist({ onOpenChart }) {
    ...
    const symbols = useMemo(() => watchlistItems.map(w => resolveTradingSymbol(w)).filter(Boolean), [watchlistItems]);
    ...
    const handleQuickTrade = async (symbol, action, livePrice, liveCurrency) => {
        ...
        const qty = parseInt(
            prompt(`Enter quantity to ${action} ${symbol} at ${displaySymbol}${livePrice.toFixed(2)}:`),
            10,
        );
        if (!qty || qty <= 0) return;
        ...
        return {
            ...
            symbol: (
                <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                    ...
                    <span className="symbol-badge" style={{ background: "none", padding: 0 }}>
                        {symbol}
                    </span>
                </div>
            ),
        ...
        },
        {
            ...
            render: (_, row) => (
                <div className="actions-row">
                    ...
                    <Button variant="ghost" size="sm" onClick={() => handleOpenChart(row.rawSymbol)}>
                        Chart
                    </Button>
                    <Button variant="ghost" size="sm" onClick={() => handleRemove(row.rawSymbol)}>
                        Remove
                    </Button>
                </div>
            ),
    ...
    return (
        <div className="page-container">
            <PageHeader
                title="Watchlist"
                description="Track your favorite symbols with live prices and quick paper trading actions."
            />
                ...
                <div className="watchlist-controls">
                    <div style={{ flex: 1, position: "relative" }}>
                        <input
                            ...
                            placeholder="Search stocks, ETFs, or indices to add to watchlist..."
                                                ...
                                                >
                                                    ...
                                                    <div style={{ minWidth: 0 }}>
                                                        ...
                                                        <div className="search-result-subtitle">
                                                            {result.company_name || result.shortname || result.symbol}
                                                        </div>
                                                    </div>
                                                </div>
                                                <Button
