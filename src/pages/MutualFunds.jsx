...
const COLLECTIONS = [
    {
        ...
        icon: (
            <svg width="28" height="28" fill="none" stroke="#00D09C" strokeWidth="1.8" viewBox="0 0 24 24">
    ...
    },
    {
        ...
        icon: (
            <svg width="28" height="28" fill="none" stroke="#6c63ff" strokeWidth="1.8" viewBox="0 0 24 24">
    ...
    },
    {
        ...
        icon: (
            <svg width="28" height="28" fill="none" stroke="#f5a623" strokeWidth="1.8" viewBox="0 0 24 24">
    ...
    },
    {
        ...
        icon: (
            <svg width="28" height="28" fill="none" stroke="#FFD700" strokeWidth="1.8" viewBox="0 0 24 24">
    ...
    },
    {
        ...
        icon: (
            <svg width="28" height="28" fill="none" stroke="#00b4d8" strokeWidth="1.8" viewBox="0 0 24 24">
    ...
    },
    {
        ...
        icon: (
            <svg width="28" height="28" fill="none" stroke="#ff6b6b" strokeWidth="1.8" viewBox="0 0 24 24">
...
const TOOLS = [
    {
        ...
        color: "#00D09C",
        icon: (
            <svg width="26" height="26" fill="none" stroke="#00D09C" strokeWidth="1.8" viewBox="0 0 24 24">
    ...
    },
    {
        ...
        color: "#6c63ff",
        icon: (
            <svg width="26" height="26" fill="none" stroke="#6c63ff" strokeWidth="1.8" viewBox="0 0 24 24">
    ...
    },
    {
        ...
        color: "#f5a623",
        icon: (
            <svg width="26" height="26" fill="none" stroke="#f5a623" strokeWidth="1.8" viewBox="0 0 24 24">
    ...
    },
    {
        ...
        color: "#00b4d8",
        icon: (
            <svg width="26" height="26" fill="none" stroke="#00b4d8" strokeWidth="1.8" viewBox="0 0 24 24">
                <polyline points="2 10 2 4 8 4" />
                <polyline points="22 14 22 20 16 20" />
                <path d="M2 4l10 10" />
                <path d="M22 20L12 10" />
            </svg>
        ),
    },
    {
        ...
        color: "#ff6b6b",
        icon: (
            <svg width="26" height="26" fill="none" stroke="#ff6b6b" strokeWidth="1.8" viewBox="0 0 24 24">
...
function getCategory(name) {
    ...
    if (
        n.includes("INDEX") ||
        (n.includes("NIFTY") && (n.includes("INDEX") || n.includes("BEES") || n.includes("ETF")))
    )
        return "Index Fund";
    ...
    if (
        ...
        return "Large Cap";
    if (n.includes("MID CAP") || n.includes("MIDCAP") || n.includes("EMERGING EQUITY") || n.includes("MID AND SMALL"))
        return "Mid Cap";
    if (n.includes("SMALL CAP") || n.includes("SMALLCAP")) return "Small Cap";
    if (n.includes("ELSS") || n.includes("TAX SAVER") || n.includes("TAX SAVINGS") || n.includes("LONG TERM EQUITY"))
        return "ELSS";
...
function shortName(schemeName) {
    return String(schemeName || "Unknown Fund")
...
function FundCard({ fund, details, onStartSip }) {
    ...
    return (
        <button
            className="gd-stock-card"
            onClick={() => onStartSip?.(fund)}
            style={{ padding: "16px", display: "flex", flexDirection: "column", textAlign: "left", minWidth: "160px" }}
        >
            <div
                className="gd-stock-icon"
                style={{ background: "linear-gradient(135deg, #1e3a5f, #0d4f3c)", color: "#7dd3c8", border: "none" }}
            >
                {ini}
            </div>
            ...
            <div className="gd-stock-price-row">
                <span className="gd-card-price">
                    {d && d.nav > 0 ? (
                        ...
                        <span className="sk-line w-20" style={{ display: "inline-block" }} />
                    )}
                </span>
            </div>
                ...
                {d ? (
                    d.ret3Y !== null && d.ret3Y !== undefined ? (
                        <span className={`gd-card-change ${d.ret3Y >= 0 ? "up" : "down"}`}>
                            {d.ret3Y >= 0 ? "+" : "−"}
                            {Math.abs(d.ret3Y).toFixed(1)}% 3Y
                        </span>
                    ) : (
                        <span className="mf-ret-dash" style={{ fontSize: 12 }}>
                            —
                        </span>
                    )
                ) : (
...
function AllFundRow({ fund, details, onStartSip }) {
    ...
    return (
        <div className="mf-all-fund-row">
            ...
            <button className="mf-sip-btn" onClick={() => onStartSip(fund)}>
                Start SIP
            </button>
        </div>
    );
...
function FundSkeleton() {
    return (
        <div className="mf-all-fund-row mf-skeleton">
            ...
            <div className="mf-all-returns">
                {[1, 2, 3].map(i => (
                    <div key={i} className="mf-ret-col">
...
export default function MutualFunds({ appPrices = {} }) {
    ...
    const handleStartSipClick = useCallback(
        fund => {
            ...
            try {
                ...
                const filtered = list
                    ...
                    .map(f => ({
                        ...
                        category: getCategory(f.schemeName),
                    }))
                ...
                if (!cancelled) {
                    ...
                    // Find popular funds
                    const pop = POPULAR_PATTERNS.map(pat =>
                        filtered.find(f => f.schemeName.toUpperCase().includes(pat.toUpperCase())),
                    )
                        .filter(Boolean)
                        .slice(0, 6);
                    setPopularFunds(pop);
    ...
    const recentFunds = useMemo(() => {
        if (recentFundRows.length > 0) {
            return recentFundRows
                .map(row =>
                    allFunds.find(
                        fund => fund.schemeCode === String(row.symbol) || fund.schemeCode === String(row.yahoo_symbol),
                    ),
                )
                .filter(Boolean);
        }
                    ...
                    {domesticIndices.map(idx => {
                        ...
                        return (
                            <div
                                ...
                                onClick={() => handleOpenChart(idx.symbol)}
                        ...
                        {searchQuery.trim().length > 0 && (
                            ...
                            >
                                {isSearching ? (
                                    ...
                                    <div style={{ padding: "20px", color: "#555", fontSize: 13, textAlign: "center" }}>
                                        No results found
                                    </div>
                                )}
                            </div>
                            ...
                            <div className="mf-collections-row">
                                {COLLECTIONS.map(col => (
                                    <button
                                        ...
                                        onClick={() => {
                                            toggleFilter(col.filterId);
                                            setActiveTab("Explore");
                                        }}
                                    >
                                        <div
                                            className="mf-col-icon-wrap"
                        ...
                        <section className="gd-explore-section">
                            ...
                            <div className="gd-stock-grid">
                                {recentFunds.length > 0 ? (
                                    ...
                                    <div className="gd-empty" style={{ padding: "24px", width: "100%" }}>
                                        Browse funds to see recently viewed.
                                    </div>
                                )}
                            </div>
                        </section>
                        ...
                        <section className="gd-explore-section">
                            ...
                            <div className="mf-all-funds-list">
                                {fundsLoading ? (
                                    ...
                                    <div className="gd-empty" style={{ padding: 40 }}>
                            ...
                            {!fundsLoading && filteredFunds.length > 0 && (
                                <p className="mf-count-note">
                                    Showing {Math.min(visibleCount, filteredFunds.length)} of{" "}
                ...
                {activeTab === "Dashboard" && (
                    <div className="gd-tab-panel">
                        <div className="gd-empty" style={{ paddingTop: 64 }}>
                            ...
                            <p style={{ fontSize: 14, maxWidth: 340, margin: "0 auto", color: "#666" }}>
                                Invest in a fund to see your portfolio here.
                            </p>
                        </div>
                    </div>
                )}
                            ...
                            >
                                <div>
                                    <div style={{ fontSize: 13, color: "#aaa", marginBottom: 4 }}>
                                        Total Active SIPs Amount / month
                                    </div>
                                    <div style={{ fontSize: 32, fontWeight: 600, color: "#fff" }}>
                                        ₹
                                        {userSips
                                            .reduce((sum, s) => sum + (s.status === "Active" ? s.amount : 0), 0)
                            ...
                            {userSips.length > 0 ? (
                                            ...
                                            <div style={{ display: "flex", gap: 14, alignItems: "center" }}>
                                                ...
                                                <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                                                    <span style={{ fontSize: 14, color: "#fff", fontWeight: 500 }}>
                                                        {shortName(sip.schemeName)}
                                                    </span>
                                                    <span style={{ fontSize: 12, color: "#00D09C" }}>
                                                        Next installment: {sip.nextPaymentDate}
                                                    </span>
                                                </div>
                                            </div>
                                            <div
                                ...
                                <div className="gd-empty" style={{ padding: "40px 0" }}>
                                    ...
                                    <p style={{ fontSize: 14, color: "#666" }}>
                                        Start a Systematic Investment Plan to automate your wealth creation.
                                    </p>
                                </div>
                            )}
                        </div>
                ...
                {activeTab === "Watchlists" && (
                    <div className="gd-tab-panel">
                        <div className="gd-empty" style={{ paddingTop: 64 }}>
                            ...
                            <p style={{ fontSize: 14, color: "#666", maxWidth: 340, margin: "0 auto" }}>
                                Add funds you like to track them here.
                            </p>
                        </div>
                    </div>
                )}
                ...
                >
                    ...
                    >
                        <button
                            ...
                            onClick={() => setSipModal({ isOpen: false, fund: null })}
                        >
                            <svg
                                width="24"
                                height="24"
                        ...
                        >
                            {shortName(sipModal.fund.schemeName)}
                        </div>
                        ...
                        <div style={{ marginBottom: 20 }}>
                            ...
                            <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
                                {[500, 1000, 2500, 5000].map(amt => (
                                    <button
                                        key={amt}
                                        onClick={() => setSipAmount(String(amt))}
                                        style={{
                                            flex: 1,
                                            padding: "6px 0",
                                            background: sipAmount === String(amt) ? "rgba(0,208,156,0.15)" : "#181818",
                                            color: sipAmount === String(amt) ? "#00D09C" : "#888",
                                            border:
                                                sipAmount === String(amt) ? "1px solid #00D09C" : "1px solid #2a2a2a",
                                            borderRadius: "6px",
                                            fontSize: 12,
                                            cursor: "pointer",
