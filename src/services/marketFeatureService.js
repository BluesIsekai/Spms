...
export function inferAssetTypeFromSymbol(symbol = '') {
  ...
  if (value.includes('=F') || ['CL=F', 'GC=F', 'SI=F', 'NG=F', 'NQ=F', 'ES=F', 'YM=F', 'RTY=F'].includes(value)) return 'FNO';
...
export async function fetchRecentViews(userId, { limit = 7, assetTypes = [] } = {}) {
  if (!requireSupabase() || !userId) return [];
...
export async function recordRecentView(userId, symbol, {
  yahooSymbol,
  companyName,
  assetType,
  sourcePage,
} = {}) {
  if (!requireSupabase() || !userId || !symbol) return null;
...
export async function upsertMutualFundSip({
  ...
  if (!requireSupabase() || !userId || !schemeCode) return null;
